...
module.exports.run = async function ({ api, event, args, Users, Currencies }) {
    ...
    switch (args[0]) {
        ...
        case 'spin': {
            ...
            var items = [`${this.checkPath(2, senderID).Island.level * 1000} coins`, `${this.checkPath(2, senderID).Island.level * 3000} coins`, `${this.checkPath(2, senderID).Island.level * 5000} coins`, 'siêu trộm', 'khiên', 'tấn công', '1 lượt quay', '2 lượt quay', '5 lư���t quay'];
    ...
    switch (handleReply.type) {
        ...
        case 'build': {
            ...
            if(a.Island.data.tower == 50 && a.Island.data.tree == 50 && a.Island.data.pool == 50 && a.Island.data.pet == 50) {
                await new Promise(resolve => setTimeout(resolve, 1000));
                api.sendMessage(`Xây dựng trên đảo bạn đã đạt được cấp tối đa!\nB���n sẽ được nâng cấp lên đảo LV ${(a.Island.level) + 1}`, threadID, messageID);
        ...
        case 'shop': {
            if(body == 1) {
                ...
                return api.sendMessage('Vui lòng reply tin nhắn này với s�� tiền bạn muốn đổi! Chiết khấu 0%', threadID, (error, info) => {
                    global.client.handleReply.push({
                        name: this.config.name,
                        messageID: info.messageID,
                        author: event.senderID,
